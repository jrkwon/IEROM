...
**
** DESCRIPTION:
**            Control code that uses :
**            TuMoC (Tubular Morphology Container) & XCATS (eXtensible
**            Comprehensive Analysis-engine for Tubular Structures)
**            Designed by Shruthi Raghavan
**
** AUTHOR:
**        Shruthi Raghavan
**
...
int main(int argc, char *argv[])
{
    ...
    bool allow_nfurc = false; //unused - for feature to be added. Harcoded Default = true.
    bool type_fix = false; //unused - for feature to be added. Hardcoded Default = false.
    if(argc<3){
        ...
        qDebug()<< ">>> Usage:" << endl;
        qDebug()<< ">>> SWC_Morphology_DS.exe -i <input_swc_file>" <<endl;
        qDebug()<< ">>> Optional Arguments with Switches:" <<endl;
        qDebug()<< ">>>   -o <output_files_path>" << endl;
        qDebug()<< ">>>   -p json" << endl;
        qDebug()<< ">>>   -c vtk" << endl;
        qDebug()<< ">>>   -vol_series (In file should be in filename_x_y_z.swc format)";
        return EXIT_SUCCESS;
    }
    else{
        ...
        for(i=1;i<argc;i++){
            ...
            if((switch_name == "-i") || (switch_name == "-I")){
                ...
                std::cout << "Unrecognized switch: " << switch_name;
                return EXIT_FAILURE;
            }
        }
    }
